...
class PartnershipService:
    ...
    async def apply(self, data: PartnershipApply, current_user: User) -> Partnership:
        # 检查项目是否存在
        ...
        if project.owner_id == current_user.id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="不能申请加入自己的项目"
            )
    ...
    async def approve(self, partnership_id: UUID, current_user: User) -> Partnership:
        partnership = await self.repo.get_by_id(partnership_id)
        if not partnership:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="申请不存在"
            )
        ...
        if project.owner_id != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="没有权限审批此申请"
            )

        if partnership.status != PartnershipStatus.PENDING:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="该申请已处理"
            )

        partnership.status = PartnershipStatus.APPROVED
        return await self.repo.update(partnership)

    async def reject(self, partnership_id: UUID, current_user: User) -> Partnership:
        partnership = await self.repo.get_by_id(partnership_id)
        if not partnership:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="申请不存在"
    ...
    async def cancel(self, partnership_id: UUID, current_user: User) -> None:
        partnership = await self.repo.get_by_id(partnership_id)
        if not partnership:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="申请不存在"
        ...
        if partnership.user_id != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="没有权限取消此申请"
            )

        if partnership.status != PartnershipStatus.PENDING:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="只能取消待审核的申请"
            )

        await self.db.delete(partnership)
        await self.db.commit()

    async def leave(self, partnership_id: UUID, current_user: User) -> Partnership:
        partnership = await self.repo.get_by_id(partnership_id)
        if not partnership:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="合伙关系不存在"
            )

        if partnership.user_id != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="没有权限操作"
            )

        if partnership.status != PartnershipStatus.APPROVED:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="您不是该项目的合伙人"
            )
    ...
    async def get_project_partnerships(
        ...
        return await self.repo.get_by_project(
            project_id, status_filter, page, page_size
        )
